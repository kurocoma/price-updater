 ...
 * 楽天 RMS 価格反映実行 API
 *
 * runId を受け取り、price_change_log の楽天 pending エントリを処理する。
 * ItemAPI (PATCH) で各商品の standardPrice を更新し、結果をログに記録する。
 */
...
export async function POST(req: NextRequest) {
  try {
    ...
    const logs = await db
      ...
      .where(
        and(
          ...
          eq(priceChangeLog.mall, "rakuten"),
    ...
    if (logs.length === 0) {
      return NextResponse.json({
        success: true,
        message: "楽天の更新対象がありません",
        updated: 0,
      });
    }

    // 更新データを構築
    const updateItems: RakutenUpdateItem[] = logs.map((log) => ({
      syohinCode: log.syohinCode,
      mallCode: log.syohinCode, // price_change_log には mallCode がないため syohinCode を使用
      newPrice: log.newPrice,
    }));

    // 楽天 API で価格更新
    const results = await executeRakutenUpdate(updateItems);
